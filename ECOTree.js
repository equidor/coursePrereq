		...
		if (!node._isAncestorCollapsed())
		{
			switch (this.render)
			{
				case "CANVAS":
					...
					//HTML part...
					s.push('<div id="' + node.id + '" class="econode" style="top:'+(node.YPosition+this.canvasoffsetTop)+'px; left:'+(node.XPosition+this.canvasoffsetLeft)+'px; width:'+node.w+'; height:'+node.h+';" ');
...
ECOTree.prototype.toString = function () {	
	...
	{
		case "CANVAS":
			s.push('<canvas id="ECOTreecanvas" width=4000 height=2000></canvas>');
			break;
